...
function preorderTraversal(root:BinaryTree<number> | null): number[] | null{
  // 関数を完成させてください
  return preorderTraversalHelper(root, []);
}

function preorderTraversalHelper(root: BinaryTree<number> | null, output: number[]): number[] | null {
  if(!root) return null;
...
// [0,-10,5,null,-3,null,9] を例に JavaScript で再帰の動きを説明します。番号は以下を意味します。
